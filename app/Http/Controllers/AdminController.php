...
public function destroyAirtickets($id)
{
    ...
    return redirect()->route('admin.airtickets')->with('success', 'Airticket deleted successfully!');
}
    ...
    public function updateGuide(Request $request, $id)
    {
        ...
        if ($request->hasFile('profileImage')) {
            ...
            if ($guide->profileImage) {
                Storage::delete($guide->profileImage);
            }

            // Store new image
            $imagePath = $request->file('profileImage')->store('profile_images', 'public');
            $guide->profileImage = 'storage/' . $imagePath;
        }
